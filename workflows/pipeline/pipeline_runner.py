"""Canonical pipeline orchestrator — checkpoints, resume, YouTube sync.

CLI, backend, and desktop all reach this via workflows/cogitator.py main().
"""
import glob
import json
import os
...
from workflows.observability import phase_timer, reset_timings
...
def _get_cogitator():
    from workflows.cogitator import (
        ...
        ASSEMBLY_DIR,
        OUTPUT_DIR,
        set_pipeline_stop_requested,
        get_pipeline_stop_requested,
        get_script_id_map,
        _refresh_learning_state,
        _cs_extract_context_from_transcript,
        _cs_update_context,
    ...
    return {
        ...
        "set_pipeline_stop_requested": set_pipeline_stop_requested,
        "get_pipeline_stop_requested": get_pipeline_stop_requested,
        "get_script_id_map": get_script_id_map,
        ...
        "phase_download": phase_download,
        "phase_transcribe": phase_transcribe,
...
def run_pipeline(skip=None):
    """Run phases 1–7 with checkpoint resume and post-run YouTube sync."""
    c = _get_cogitator()
    c["set_pipeline_stop_requested"](False)

    def check_stop():
        if c["get_pipeline_stop_requested"]():
    ...
    try:
        ...
        c["log"](f"Retention history load skipped: {e}")

    skip = set(skip or set())

    if 1 not in skip:
        with phase_timer(1, "download", c["log"]):
            c["phase_download"]()
        if check_stop():
            return
    ...
    if checkpoint:
        ...
        if not skip:
            skip = completed
            c["log"](
                f"Checkpoint found: phases {sorted(completed)} completed, "
    ...
    json_file = None
    if 2 not in skip:
        with phase_timer(2, "transcribe", c["log"]):
        ...
        if not os.path.exists(json_file):
            ...
            json_file = (
                sorted(existing, key=os.path.getmtime, reverse=True)[0]
                if existing
                else None
            )

    if 3 not in skip:
        with phase_timer(3, "context", c["log"]):
            c["phase_context"]()
        if check_stop():
            return
        ...
        try:
            ...
            if transcript_text:
                ...
                extracted = c["_cs_extract_context_from_transcript"](
                    transcript_text[:10000], game_title
                )
                if extracted:
                    ctx = c["_cs_update_context"](extracted, os.path.basename(json_file))
                    verified = c["load_verified_context"](game_title)
                    final = c["merge_context_dicts"](
                        verified.get("context", {}) if verified else {}, ctx
                    )
                    c["save_verified_context"](game_title, final)
                    c["compute_and_save_implicit_relationships"](
                        game_title, transcript_text
                    )
                    c["log"](
                        f"Context extracted: {len(final.get('characters', []))} chars, "
                        f"{len(final.get('relationships', []))} rels"
                    )
        except Exception as e:
    ...
    max_shorts = max(
        1, duration // interval + (1 if duration % interval > interval // 2 else 0)
    )
    if num_shorts <= 0:
    ...
    if json_file:
        ...
        for s in selected:
            c["log"](
                f"  Interval {s['index']+1}: {s['start']//60}-{s['end']//60}min "
                f"(score: {s['score']:.1f})"
            )
    else:
        selected = [
    ...
    c["log"](
        f"Video: {duration}s = {max_shorts} interval(s), generating {len(selected)} short(s)"
    )

    if 4 not in skip and json_file:
        with phase_timer(4, "scripts", c["log"]):
    ...
    if 5 not in skip and json_file:
        with phase_timer(5, "clips", c["log"]):
            c["phase_clips"](
                video, json_file, duration, selected, script_id_map=c["get_script_id_map"]()
            )
        if check_stop():
            return
    ...
    if 7 not in skip:
        with phase_timer(7, "assemble", c["log"]):
...
Total output files: {sc + cc + tw + ts + ac}"""
    ...
    try:
        ...
        c["log"](
            f"YouTube sync: {sync_r.get('matched_count', 0)} matched, "
            f"{sync_r.get('new_metrics', 0)} new metrics"
        )
        if train_r.get("success"):
            c["log"](
                f"Model trained: {train_r.get('sample_count', 0)} samples, "
    ...
    try:
        ...
        tk = auto_import_tiktok()
        c["log"](f"TikTok auto-import: {tk.get('count', 0)} new files")
    except Exception as e:
        c["log"](f"TikTok auto-import skipped: {e}")


def run_local_pipeline(path, skip=None):
    """Process local recordings via the same orchestrator (skip download)."""
    from workflows.cogitator import run_local_recordings

    # Local path still uses multi-file loop in cogitator; each file runs phases 2–7.
    # Prefer skip download when calling full run_pipeline after copy.
    skip = set(skip or set()) | {1}
